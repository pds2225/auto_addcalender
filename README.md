...
일반 일정은 모바일에서도 `내 캘린더`에 등록되고, 제목이나 메모에 `마감`이 있으면 `업무마감일` 캘린더로 열립니다.
...
5. 일정이 여러 개면 원하는 일정만 체크한 뒤 `선택 일괄 열기`를 누릅니다. PC는 짧은 간격으로 이어서 열고, 안드로이드 휴대폰은 버튼을 한 번 누를 때마다 일정 1개를 엽니다. 개별 등록도 가능합니다.
