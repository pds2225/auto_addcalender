...
from bulk_open import build_bulk_open_page, bulk_open_iframe_height
...
- 모집·신청·접수 마감 일정은 제목 맨 앞에 반드시 "[마감] "을 붙일 것
...
- "모집기간", "신청기간", "접수기간"처럼 시작일~종료일 범위가 있으면 종료 시점을 신청 마감으로 해석하고 "[마감] " 이벤트를 반드시 별도로 1개 생성. 기간 전체를 마감 이벤트로 만들지 말 것
- 같은 이미지/문서에 서류평가 결과발표, 본선, 면접, 시상식 등 다른 일정이 함께 있어도 모집·신청·접수 마감 이벤트를 누락하거나 다른 날짜로 대체하지 말 것
- 예: "2026. 9. 14(월) ~ 10. 6(화) 18시까지" → 마감일 2026-10-06 18:00. 종료일 연도가 생략되면 시작일 연도 상속, 시작 월보다 종료 월이 작으면 다음 연도로 해석
- "18시까지", "18:00까지", "오후 6시까지" 등 명시 시간이 있으면 해당 시간을 정확히 사용
...
[장소 규칙]
- 공고 원문 URL이 있으면 모든 일정의 location에는 공고 원문 URL을 그대로 넣는다. query parameter를 삭제하거나 축약하지 말 것
- 공고 원문 URL이 있으면서 실제 방문/개최 장소가 있으면 실제 장소(주소·건물명·층·호실·강의실)는 details에 보존한다
- 공고 원문 URL이 있으면서 Zoom/Meet 등 별도 접속 링크가 있으면 접속 링크는 details에 보존한다
- 공고 원문 URL이 없는 경우에만 오프라인 행사의 location에 실제 장소를 넣고, 온라인 회의는 해당 접속 링크를 location에 넣는다
- 기관명만 있고 주소가 없으면, 그 기관이 실제 방문/개최 장소일 때만 실제 장소 정보로 취급
- 제목·프로그램·주최·주관에 나온 기관명만으로 location을 채우지 말 것
- 입주·입사·선정·지원 대상 시설명(예: "서울AI허브 입주기업 모집"의 AI허브)은 행사 장소가 아님
- "온라인" 자체를 location에 넣지 말 것
...
- 우선순위: ① 실제 방문 장소/주소(있을 때만) ② 모집·접수 방법(온라인 접수 등) ③ 문의처·연락처·전화번호·이메일 ④ 준비물·지참 서류 ⑤ 주요 세션명·일정 흐름
- 실제 방문 장소가 없으면 ①을 건너뛰고 모집·접수 방법부터 요약
...
def process_image(image_bytes, mime_type, supplemental_text=""):
    ...
    current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    source_lines = [
        "아래 이미지에서 일정을 추출하여 JSON 배열로만 출력하십시오.",
        "특히 이미지에 모집기간·신청기간·접수기간이 보이면 그 종료 시점을 '[마감] ' 이벤트로 반드시 포함하십시오.",
        "결과발표·본선·면접·시상식 등 다른 일정이 있어도 신청 마감일을 누락하지 마십시오.",
        "종료일 연도가 생략되면 시작일 연도를 상속하고, 명시된 마감시간(예: 18시까지)을 정확히 사용하십시오.",
    ]
    if normalized_text:
        source_lines.extend(
            [
...
def render_bulk_register_section(events, selected_platforms):
    """복수 일정에서 원하는 항목만 체크해 구글 캘린더 탭을 순서대로 연다."""
    ...
    components.html(
        build_bulk_open_page(items),
        height=bulk_open_iframe_height(len(events)),
    )
